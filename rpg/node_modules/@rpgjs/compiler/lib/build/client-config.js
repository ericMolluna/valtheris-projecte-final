...
export async function clientBuildConfig(dirname, options = {}, config) {
    ...
    const outputDir = buildOptions.outputDir;
    ...
    if (!isServer) {
        ...
        if (!isBuild) {
            plugins.push(mapUpdatePlugin(isRpg ? '' : serverUrl, config));
        }
    }
    if (isBuild && !isTest) {
        plugins.push(tmxTsxMoverPlugin(isRpg ? outputDir : join(outputDir, 'server')), mapExtractPlugin(dirOutputName));
    }
    ...
    return defaultComposer(viteConfig, vite);
}
