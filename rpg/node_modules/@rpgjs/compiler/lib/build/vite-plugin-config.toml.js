...
export function formatVariableName(packageName) {
    packageName = packageName.replace(/\./g, '');
    return packageName.replace(/[.@\/ -]/g, '_');
}
export function transformPathIfModule(moduleName) {
...
export function searchFolderAndTransformToImportString(folderPath, modulePath, extensionFilter, returnCb, options) {
    let importString = '';
...
export function importString(modulePath, fileName, variableName) {
...
export function loadServerFiles(modulePath, options, config) {
    ...
    const verifyDefaultExport = (importObject) => dd `
...
export function loadSpriteSheet(directoryName, modulePath, options, warning = false) {
...
export function loadClientFiles(modulePath, options, config) {
...
export function createModuleLoad(id, variableName, modulePath, options, config) {
...
export function createConfigFiles(id, configServer, configClient) {
...
export default function configTomlPlugin(options = {}, config) {
    let modules = [];
    let modulesCreated = [];
    ...
    return {
        ...
        async resolveId(source, importer) {
            ...
            if ((source.includes('virtual') && (!source.endsWith('virtual-server.ts') && options.serveMode)) ||
...
//# sourceMappingURL=vite-plugin-config.toml.js.map